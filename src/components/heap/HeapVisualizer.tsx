...
import { HeapNode, HeapEdge } from './HeapNode.js';
...
export function HeapVisualizer({
    ...
    const nodePositions = heapState.data.map((_, index) =>
        BinaryHeapUtils.calculateNodePosition(index, containerWidth, containerHeight)
    );

    // Generate edges
    const edges = heapState.data.reduce<Array<{
        parentPos: { x: number; y: number };
        childPos: { x: number; y: number };
        isHighlighted: boolean;
        key: string;
    }>>((acc, _, index) => {
        const children = BinaryHeapUtils.getChildrenIndices(index, heapState.data.length);

        for (const childIndex of children) {
            const isHighlighted =
                (heapState.activeIndex === index && heapState.childIndices.includes(childIndex)) ||
                (heapState.activeIndex === childIndex && heapState.parentIndex === index) ||
